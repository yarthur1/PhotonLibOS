...
public:
    ...
    KernelSocketServer(bool autoremove = false) : m_autoremove(autoremove) { }   // ExampleServer autoremove=false nonblocking=ture
...
protected:
    ...
    int accept_loop() {
        ...
        while (workth) {
            waiting = true;
            auto connection = accept();   // 返回 accept fd创建的KernelSocketStream 
            waiting = false;
            if (!workth) return 0;
            if (connection) {

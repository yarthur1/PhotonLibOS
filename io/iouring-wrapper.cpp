...
public:
    ...
    ssize_t wait_for_events(void** data, size_t count, Timeout timeout) override {  // eventfd
        // Use master engine to wait for self event fd
        int ret = ::photon::wait_for_fd_readable(m_eventfd, timeout);  // 会提交io请求
        if (ret < 0) {
    ...
    ssize_t wait_and_fire_events(uint64_t timeout) override {
        ...
        if (m_submit_wait_func(m_ring, &ts) != 0) {  // 批量提交，等待请求完成
            return -1;
        }
